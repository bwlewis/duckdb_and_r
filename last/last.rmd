...
Let's try that last SQL query using SQLite database engine to see if it works.
```{r, eval = FALSE}
library(RSQLite)
...
# [1] TRUE
```
Aha success! But still very slow. I wonder if DuckDB's silent
mis-understanding of an R type is a bug? It would probably be
better to stop with an error or at least display a warning.
I'll report that to the authors.
