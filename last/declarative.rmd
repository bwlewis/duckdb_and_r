...
blush. SQL can be a mess, is very had to compose, and sometimes makes it
difficult to express even seemingly simple objectives like the one above.

My advice for most people is this: just use dplyr if you can. Don't use SQL.
Try to think about your data munging problems the dplyr way.  For anything
other than the most basic operations, I think dplyr is a better through-out
grammar of data manipulation than SQL.  And besides, dplyr can usually front
...
solution to every problem. Indeed, dplyr seems to fail to figure out how to
translate even this simple problem for back-end databases (probably because the
SQL is so awful). The dplyr approach also brings with it considerable
complexity for some operations, although that has been improving over the
years. The fact is plain old base R often works really well in many cases,
including for this problem. And even SQL, despite my misgivings about it, has
many admirable qualities and tons of folks know it very well (surely way better
than I do) and use it productively. Use the tools you feel comfortable with.

See my notes here for more examples on this topic:
