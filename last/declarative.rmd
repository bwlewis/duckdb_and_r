...
SQL is billed as a language of declarative data manipulation and analysis. But even
for some simple tasks, SQL feels more imperative to me than alternatives like R.
Consider the following quite simple example using a large table that starts out
like:
...
The task is very simply stated:
...
```{r, eval=FALSE}
example %>% group_by(company) %>% summarize(value = last(value, order_by = date))
```
...
for each row in example table
  if i = 1 then keep row
end for
```

## SQL Oh My

As far as I can tell, a standard SQL approach to solving this task looks remarkably
like the latter, less efficient, pseudocode above:

```
SELECT company, value FROM
  (SELECT company, date_numeric, value, ROW_NUMBER() OVER
    (PARTITION BY company ORDER BY date_numeric DESC) AS i FROM example) AS yikes
  WHERE i = 1
```

What's worse, query optimizers for at least a few SQL database engines I tried
quite literally interpret the SQL code and actually do loop over the data
multiple times. See 
https://bwlewis.github.io/duckdb_and_r/last/last.html
for representative, and disasterously bad, SQL database engine timings.
...
Those discussions are painful to read through but darkly funny.


## Which approach looks more declarative to you?

Yeah. Look, I get it. SQL can be really nice for simple, basic aggregation and
filtering.  It's also really good at imposing constraints on data.  But hey, I
used to work at a database company and I've seen SQL queries that would make
Mike Stonebraker blush. SQL can be a mess, is very had to compose, and
sometimes makes it difficult to express even seemingly simple objectives like
the one above.

My advice for most people is this: just use dplyr if you can. Don't use SQL.
Think about your data munging problems the dplyr way.  For anything other than
the most basic operations, dplyr is a better through-out grammar of data
manipulation than SQL. Dplyr implements and extends ideas of Codd's relational
algebra better than SQL does (hey, it's just, like, my opinion, man).  And
besides, dplyr can be a front-end for your database and take advantage of
available optimizations provided by the database.
