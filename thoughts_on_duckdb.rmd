...
These notes present 5 easy examples that illustrate database-like tasks in R
that illustrate stylistic and performance across different R-centric and SQL
approaches to representative problems.  The examples are short and range from
basic data processing that SQL excels at to somewhat more complex, but common,
specialized workflows from genomics and finance.
...
* Use DuckDB with dplyr, avoid SQL except for basic.
...
* The TPCH join-aggregate example from DuckDB
* An example from some recent work that picks last items in groups
* Overlap joins on genomic data
* Simple grouped aggregation on large out of core data
* "As of" joins (common in finance, time series, etc.)
